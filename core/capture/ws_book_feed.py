...
in a daemon thread so the SYNCHRONOUS maker loop (``core.maker.lp_live``) can read a real-time
...
``top()`` mirrors ``core.maker.lp_pilot.best_bid_ask`` exactly — ``(bid, ask)`` in dollars with
