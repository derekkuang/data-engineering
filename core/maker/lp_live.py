...
This places REAL orders. It is the validated paper v2 control logic (core/maker/lp_pilot)
...
Usage:
    ...
    uv run python -m core.maker.lp_live --test-order              # auto-picks a market
    uv run python -m core.maker.lp_live --live --i-understand-live --minutes 15
    uv run python -m core.maker.lp_live --live --i-understand-live --prefix KXWC --ws  # WS-fed book

WebSocket mode (--live --ws): runs THIS SAME strategy but sources market data over the WS
(ingestion/kalshi_ws via core/capture/ws_book_feed) instead of REST polling — the per-poll order
