...
pick-off)? — needs a live-game flow log (ml.lp.ws_logger during a match); this does not test it.

Read-only, no auth. Numbers are a snapshot; run during evening games (US) when club books are live.

Usage:
    uv run python -m ml.lp.soccer_screen
    uv run python -m ml.lp.soccer_screen --leagues WC,MLS,BRA,MEX
"""
...
def main() -> int:
    ...
    print("\nRead: compare each league to WC. A league with a WC-like near-money spread (~2-4c),\n"
          ...
          "live in-play flow log (ml.lp.ws_logger --prefix <SERIES> --minutes 30 in a match) to\n"
