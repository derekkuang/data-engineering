...
Usage:
    uv run python -m ml.lp.breakeven                      # live league spreads
    uv run python -m ml.lp.breakeven --offline            # 07-15 snapshot spreads only
    uv run python -m ml.lp.breakeven --leagues MLS,MEX,BRA
"""
...
def live_spreads(leagues: list[str]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    from ml.lp.soccer_screen import screen_series
