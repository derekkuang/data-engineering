...
v1 (ml/lp_paper_pilot.py) showed a benign retail market (markout ~0, ~1c/fill) but
...
Usage:
    uv run python -m ml.lp.lp_pilot --minutes 20
    uv run python -m ml.lp.lp_pilot --ticker KX... --minutes 30
"""
...
def pick_smooth_ticker(
    ...
    The SELECTION GATE (``ml.lp.lp_gate.passes_gate``) drops structurally-toxic types (ITF)
