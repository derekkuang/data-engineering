...
Two measurements, per decision minute k in {1,5,10,13}:
  ...
  2. TRADEABILITY (cost-aware). Bet the perp-implied side whenever it clears the
     ...
     (reuses ml.alpha.backtest money-math). Day-block bootstrap CI + no-skill controls.
...
Usage:
    uv run python -m ml.research.binary_perp_basis                 # last ~10 days
    uv run python -m ml.research.binary_perp_basis --start 2026-06-04 --end 2026-06-14
"""
...
from ml.alpha.backtest import _per_window_pnl, _summarise
