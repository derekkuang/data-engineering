...
**Where this leaves the plan.** Step 0 ✓ and Step 3 ✓. The most logical next move is DEPTH not breadth: a small **live MEX SPREAD pilot** (best live candidate — 2c spread, ~11.6k vol, robust predicted net) via `--pilot KXLIGAMX`, with paired `ws_features` capture of the same book — one session yields realized capture-efficiency + goal-jump toxicity + the July overlap the public-vs-private correlation (roadmap Step 7) has been waiting for. Step 2 (jump/news-toxicity axis — the flow-independent metric that can actually SEE goal pick-off) is the parallel track that makes PASSIVE league screening valid, and is what earns the right to collect broadly later. Breadth (all leagues) is a SCALING move for after one book confirms, and needs the fixture-aware cron + Actions-minutes fix first.
