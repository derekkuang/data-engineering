...
**Where this leaves the plan.** Steps 0 ✓, 1 ✓, 2 ✓, 3 ✓ — the whole edge-decision architecture the review flagged is now rebuilt and honest. The next move is the small **live MEX SPREAD pilot** (`--pilot KXLIGAMX` + paired `ws_features` capture), waiting only on a live Liga MX game (evenings US; the Guadalajara fixture is the highest-volume target). One session yields realized capture-efficiency + goal-jump toxicity (now measurable) + the first July overlap for the public-vs-private correlation (Step 7). Remaining backlog: (4) stats hardening (ET `capture_day`, unweighted per-day means, raise the day floor), (5) ops (empty-capture freshness alarm + `if: always()`), (6) cost (make `fct_ws_markout`/`fct_toxicity_by_family` incremental before ~Oct). Breadth (all leagues) is a SCALING move for after one book confirms.
